...
# 0.0이면 스케줄러 없이 고정 학습률이다.
#
# 기본값은 0(스케줄러 없음)이다. validation에서 warmup 6개 구성을
# 비교해 warmup 0.1 + lr 5e-5가 가장 좋아 보였으나(AUC 0.9921 ->
# 0.9951, thr95 0.952 -> 0.9727), 판정셋으로 확인하니 오히려 나빠졌다:
# uncertain_normal_share 0.1908 -> 0.2928, alert_false_positive_rate는
# 0.0033 -> 0.0230으로 기준까지 넘겼다.
#
# 원인은 validation이 판정셋의 대리치가 못 된다는 데 있다. validation은
# 정상 92건뿐인 데다 학습 pool과 같은 출처라 이미 포화 상태고(정상
# 96.7~98.9%가 경계 아래), 구성 간 차이가 표본 1건 수준이라 사실상
# 노이즈를 고른 셈이었다. 실제 수집 문자인 판정셋에서는 정상의
# 63~81%만 내려간다 - 난이도가 근본적으로 다르다.
#
# 스케줄러 자체는 남겨둔다. 하이퍼파라미터를 다시 볼 때는 validation이
# 아니라 판정셋과 난이도가 비슷한 선정용 셋을 먼저 확보해야 한다.
