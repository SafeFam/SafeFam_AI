...
class NaiveBayesPhishingClassifier(
    ...
    def __init__(
        ...
        *,
        ...
        if calibration_cv < 2:
            raise ValueError(
                "calibration_cv must be at least 2"
            )

        self.include_structural_features = (
    ...
    def _validate_dataframe(
        ...
        *,
        ...
        if missing:
            raise ValueError(
                f"missing Naive Bayes columns: {missing}"
            )

        if df.empty:
            raise ValueError(
                "Naive Bayes input DataFrame is empty"
            )

        if df[list(required_columns)].isna().any().any():
            raise ValueError(
                "Naive Bayes input contains missing values"
            )
        ...
        if require_label:
            ...
            if not labels.issubset(
                ...
                raise ValueError(
                    f"unsupported labels: "
                    f"{labels - {'normal', 'phishing'}}"
                )

            if labels != {"normal", "phishing"}:
                raise ValueError(
                    "training data must contain both "
                    "normal and phishing labels"
                )
    ...
    def fit(
        ...
        if "phishing" not in self.classes_:
            raise RuntimeError(
                "trained model does not contain phishing class"
            )
    ...
    def _require_fitted(self) -> None:
        ...
        if self.model is None or self.classes_ is None:
            raise RuntimeError(
                "Naive Bayes model is not fitted"
            )
    ...
    def predict_scores(
        ...
        phishing_index = self.classes_.index(
            "phishing"
        )
