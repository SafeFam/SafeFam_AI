...
class RiskScoringEngine:
    """
    3중 가중치 스코어링 시스템
    - LLM 문맥 분석 — 자체 모델(1차) + 외부 LLM(2차) 하이브리드 결합 점수
    ...
    # 텍스트 트랙 내부 하이브리드 결합 가중치 (자체 모델 + LLM 둘 다 수행된 경우에만 적용)
    NAIVE_BAYES_WEIGHT = 0.3
    LLM_WEIGHT = 0.7

    # 자체 모델과 LLM이 동시에 실패해 텍스트 트랙에 아무 신호도 없을 때의 대체 점수.
    ...
    def _combine_text_track_score(
        ...
        """
        자체 모델(1차) + 외부 LLM(2차) 텍스트 위험도를 하나의 점수로 결합.
        - 자체 모델이 SAFE로 판정해 LLM을 스킵한 경우: 자체 모델 점수를 그대로 사용
        - 자체 모델 로드에 실패했지만 LLM은 정상 수행된 경우: LLM 점수를 그대로 신뢰
        - 자체 모델과 LLM이 동시에 실패한 경우: 신뢰할 수 있는 신호가 전혀 없으므로 llm_score(보통 0)를
          그대로 반환하지 않고 대체 점수로 fail-safe 처리 (두 분류기가 동시에 다운됐다는 이유만으로
          "안전"으로 오판되는 것을 방지)
        - LLM 호출이 실패한 경우(자체 모델은 정상): 이미 의심 구간으로 에스컬레이션한 상황이므로
          LLM 장애를 이유로 점수를 0으로 낮추지 않고 자체 모델 점수를 그대로 신뢰
        - 둘 다 정상 수행된 경우: LLM 문맥 분석에 더 큰 가중치를 두고 자체 모델 신호를 보조적으로 반영
        """
