...
_RE_ACCOUNT_NUMBER = re.compile(r"(?<!\d)\d{2,6}-\d{2,6}-\d{2,8}(?!\d)|(?<!\d)\d{10,14}(?!\d)")

# 카드번호: 4자리씩 4묶음(하이픈/공백 구분 또는 연속 16자리)
_RE_CARD_NUMBER = re.compile(r"(?<!\d)\d{4}[-\s]\d{4}[-\s]\d{4}[-\s]\d{4}(?!\d)|(?<!\d)\d{16}(?!\d)")

# 금융감독원 등록 기준 주요 금융기관/공공기관 명칭 (사칭 대상으로 흔히 악용되는 목록)
FINANCIAL_INSTITUTIONS = [
    # 은행
    "국민은행", "KB국민은행", "신한은행", "우리은행", "하나은행", "KEB하나은행",
    "NH농협은행", "농협은행", "IBK기업은행", "기업은행", "SC제일은행",
    "한국씨티은행", "씨티은행", "케이뱅크", "카카오뱅크", "토스뱅크",
    "수협은행", "산업은행", "KDB산업은행", "새마을금고", "신협", "우체국",
    # 카드사
    "신한카드", "삼성카드", "현대카드", "롯데카드", "하나카드",
    "KB국민카드", "국민카드", "우리카드", "BC카드", "비씨카드", "NH농협카드", "씨티카드",
    # 증권/보험
    "미래에셋증권", "삼성증권", "한국투자증권", "NH투자증권", "키움증권",
    "삼성생명", "교보생명", "한화생명",
    # 공공/사법기관 (사칭 빈도가 높은 기관)
    "금융감독원", "금융위원회", "검찰청", "경찰청", "국세청", "관세청",
    "법원", "대법원", "건강보험공단", "국민건강보험공단", "우정사업본부",
]
...
def _check_malicious_domain(traced_url: Optional[str]) -> bool:
    if not traced_url:
        return False
...
def analyze_text_with_rules(text: str, traced_url: Optional[str] = None) -> dict:
    ...
    if matched_rules:
        logger.info(f"[RuleEngine] 규칙 매치 - 점수: {rule_score} | 사유: {matched_rules}")
