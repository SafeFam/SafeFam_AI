...
def test_disabled_consumer_does_not_connect_to_rabbitmq():
    ...
    application = create_app(rabbitmq_consumer_enabled=False)

    with (
        patch("app.main.RabbitMQConnection") as mock_connection_class,
...
def test_lifespan_starts_and_stops_rabbitmq_consumer():
    ...
    fake_connection.get_request_queue.return_value = fake_queue
    fake_exchange = MagicMock()
    fake_connection.get_exchange.return_value = fake_exchange
    ...
    application = create_app(rabbitmq_consumer_enabled=True)
    ...
    with (
        ...
        consumer_arguments = mock_consumer_class.call_args.kwargs
        assert consumer_arguments["request_queue"] is fake_queue
        assert consumer_arguments["result_publisher"] is fake_result_publisher
        assert consumer_arguments["result_factory"] is fake_result_factory
        assert consumer_arguments["dead_letter_publisher"] is fake_dead_letter_publisher
...
def test_lifespan_closes_connection_when_consumer_start_fails():
    ...
    fake_connection.connect = AsyncMock()
    fake_connection.close = AsyncMock()
    fake_connection.get_request_queue.return_value = MagicMock()

    fake_consumer = MagicMock()
    fake_consumer.start = AsyncMock(side_effect=RuntimeError("Consumer start failed"))
    fake_consumer.stop = AsyncMock()
...
def test_lifespan_closes_connection_when_consumer_stop_fails():
    ...
    fake_consumer = MagicMock()
    fake_consumer.start = AsyncMock()
    fake_consumer.stop = AsyncMock(side_effect=RuntimeError("Consumer stop failed"))
