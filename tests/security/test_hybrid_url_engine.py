...
async def test_gsb_block_marks_is_gsb_confirmed_true_and_skips_vt():
    ...
    """
    engine = HybridUrlEngine()
    engine.gsb_engine.scan_url = AsyncMock(return_value={"is_malicious": True, "raw_score": 0.95})
    engine.vt_engine.scan_url = AsyncMock()

    result = await engine.scan_url("https://danger-phishing-test-site.com")

    assert result["is_malicious"] is True
    assert result["is_gsb_confirmed"] is True
    engine.vt_engine.scan_url.assert_not_called()
...
async def test_vt_only_detection_does_not_mark_gsb_confirmed():
    ...
    """
    engine = HybridUrlEngine()
    engine.gsb_engine.scan_url = AsyncMock(return_value={"is_malicious": False})
    engine.vt_engine.scan_url = AsyncMock(return_value={"is_malicious": True, "detected_count": 5, "raw_score": 0.8})
...
async def test_vt_weak_detection_below_threshold_is_not_confirmed():
    ...
    engine = HybridUrlEngine()
    engine.gsb_engine.scan_url = AsyncMock(return_value={"is_malicious": False})
    engine.vt_engine.scan_url = AsyncMock(return_value={"is_malicious": True, "detected_count": 2, "raw_score": 0.3})
...
async def test_vt_strong_consensus_at_or_above_threshold_is_confirmed():
    ...
    engine = HybridUrlEngine()
    engine.gsb_engine.scan_url = AsyncMock(return_value={"is_malicious": False})
    engine.vt_engine.scan_url = AsyncMock(return_value={"is_malicious": True, "detected_count": 7, "raw_score": 0.9})
...
async def test_clean_url_is_not_gsb_confirmed():
    engine = HybridUrlEngine()
    engine.gsb_engine.scan_url = AsyncMock(return_value={"is_malicious": False})
    engine.vt_engine.scan_url = AsyncMock(return_value={"is_malicious": False, "detected_count": 0})
