...
@pytest.mark.asyncio
@patch("app.service.security.gemini_text_analyzer.is_mock_enabled", return_value=True)
async def test_analyze_text_with_gemini_mock_mode(mock_is_enabled):
    """
    Mock 스위치가 켜졌을 때 실제 Gemini API를 호출하는 대신 mock_provider의 사전 정의 데이터로
    ...
    assert result["result"]["risk_score"] == 96
    assert len(result["result"]["evidence"]) > 0


@pytest.mark.asyncio
@patch("app.service.security.gemini_text_analyzer.is_mock_enabled", return_value=True)
async def test_analyze_text_with_gemini_mock_default_safe(mock_is_enabled):
    """
    Mock DB에 정의되지 않은 임의의 텍스트가 들어왔을 때 기본 안전 응답으로 처리되는지 검증합니다.
    """
...
@pytest.mark.asyncio
@patch("app.service.security.gemini_text_analyzer.is_mock_enabled", return_value=False)
@patch("app.service.security.gemini_text_analyzer.GEMINI_API_KEY", None)
async def test_analyze_text_with_gemini_missing_api_key(mock_is_enabled, mock_key=None):
    """
