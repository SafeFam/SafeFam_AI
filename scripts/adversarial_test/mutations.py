...
async def mutate_tone_normalization(text: str) -> str:
    return await _paraphrase(
        ...
        "아래 요소를 최대한 활용해 '진짜 은행/기관 안내문'처럼 보이게 하라 "
        "(SafeFam 실측 평가에서 문맥분석을 가장 잘 통과시킨 패턴들이다):\n"
        "- '안녕하십니까, OOO입니다' 같은 격식체 인사말로 시작\n"
        "- '항상 이용해 주셔서 감사합니다' 류의 감사 인사 삽입\n"
        "- 핵심 정보(예금주/계좌번호/결제금액 등)를 '■' 불릿과 '항목: 값' 형태로 구조화해 나열\n"
        "- 요청 문장은 전부 '~하여 주시기 바랍니다', '~해 주시기 바랍니다' 같은 정중한 청유형 어미 사용\n"
        "- 연락처는 '고객센터', '전담 신고센터', '문의처' 등 공식적인 창구 명칭으로 표현\n"
        "- 끝맺음에 '감사합니다', '안전한 금융거래를 위해 최선을 다하겠습니다' 같은 정중한 마무리 인사 추가",
    )
